...
/**
 *   	\file       bookmark_list.php
 *		\ingroup    mastereditor
 *		\brief      List page for bookmark
 */
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->rights->mastereditor->bookmark->write;
	$permissiontodelete = $user->rights->mastereditor->bookmark->delete;
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($conf->mastereditor->enabled)) accessforbidden('Module not enabled');
...
if (empty($reshook)) {
	...
	$uploaddir = $conf->mastereditor->dir_output;
...
//$help_url = "EN:Module_Bookmark|FR:Module_Bookmark_FR|ES:Módulo_Bookmark";
$help_url = '';
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/mastereditor/bookmark_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/mastereditor/bookmark_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				if ($key == 'status') {
					...
					$url = $object->showOutputField($val, $key, $object->$key, '');
					print '<a href="' . $url . '">' . $url . '</a>';
					//var_dump($object->showOutputField($val, $key, $object->$key, ''));
				} else {
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_mastereditor', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
